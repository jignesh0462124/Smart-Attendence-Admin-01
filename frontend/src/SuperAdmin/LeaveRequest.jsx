...
const superLogout = () => { console.log("Logging out..."); }; 
...
export default function LeaveRequests() {
  ...
  async function fetchLeaveRequests() {
    // Mock Data: Added a 3rd 'Pending' item (Mark Green) to show the 3rd card
    const mockData = [
        { id: 1, employee_name: "John Smith", role: "Developer", leave_type: "Vacation", start_date: "2024-11-01", end_date: "2024-11-05", reason: "Annual holiday trip.", status: "Pending", avatar_url: "https://ui-avatars.com/api/?name=John+Smith&background=random", total_allocated_leaves: 24, leaves_used_so_far: 12 },
        { id: 2, employee_name: "Alice Brown", role: "UX Designer", leave_type: "Sick Leave", start_date: "2024-10-25", end_date: "2024-10-25", reason: "Flu symptoms.", status: "Pending", avatar_url: "https://ui-avatars.com/api/?name=Alice+Brown&background=random", total_allocated_leaves: 20, leaves_used_so_far: 18 },
        // Changed Mark Green to 'Pending' so he appears as the 3rd card
        { id: 3, employee_name: "Mark Green", role: "Product Manager", leave_type: "Personal", start_date: "2024-10-15", end_date: "2024-10-15", reason: "Family event.", status: "Pending", avatar_url: "https://ui-avatars.com/api/?name=Mark+Green&background=random", total_allocated_leaves: 15, leaves_used_so_far: 5 },
        { id: 4, employee_name: "Emily Clark", role: "HR Specialist", leave_type: "Maternity", start_date: "2024-09-01", end_date: "2025-01-01", reason: "Maternity leave.", status: "Approved", avatar_url: "https://ui-avatars.com/api/?name=Emily+Clark&background=random", total_allocated_leaves: 90, leaves_used_so_far: 0 },
        { id: 5, employee_name: "David Lee", role: "Analyst", leave_type: "Vacation", start_date: "2024-10-01", end_date: "2024-10-03", reason: "Long weekend getaway.", status: "Rejected", avatar_url: "https://ui-avatars.com/api/?name=David+Lee&background=random", total_allocated_leaves: 24, leaves_used_so_far: 20 },
    ];
                  ...
                  <div className="flex flex-col gap-5">
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                          {loading ? <p className="col-span-full text-center text-slate-500">Loading requests...</p> : pendingRequests.slice(0, 3).map((req) => {
                              ...
                              const total = req.total_allocated_leaves || 0;
                              const used = req.leaves_used_so_far || 0;
                              ...
                              return (
                                  <div key={req.id} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group relative overflow-hidden">
                                      {/* Colored Side Border */}
                                      <div className={`absolute top-0 left-0 w-1.5 h-full ${theme.border}`}></div>
                                      
                                      <div className="p-6 flex flex-col h-full pl-7"> {/* Increased padding-left for border space */}
                                          <div className="flex justify-between items-start mb-5">
                                              ...
                                              {/* --- CHANGED: Stats now under the Badge --- */}
                                              <div className="flex flex-col items-end gap-1.5">
                                                  ...
                                                  <span className="text-[11px] font-semibold text-slate-500 bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
                                                      Remaining: <span className={remaining < 5 ? "text-red-500" : "text-slate-700"}>{remaining}</span> / Total: {total}
                                                  </span>
                                              </div>
                                              {/* -------------------------------------- */}
                                          </div>
